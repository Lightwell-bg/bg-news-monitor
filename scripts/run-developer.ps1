...
try {
    ...
    $logPath = Join-Path -Path $runsPath -ChildPath ('developer-{0}.txt' -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Выполни только поставленную задачу как Developer. Запусти обязательные проверки и верни короткий проверяемый отчёт.
...
"@
    ...
    & $claudeCommand.Source -p --agent developer --permission-mode auto --no-session-persistence $prompt 2>&1 |
